...
CodexGauge is that reminder: a tiny, always-on-top widget that quietly shows your current `5h` and weekly usage, refreshes itself adaptively, and shifts colour as you near the limit, so you always know how much runway you have left.
...
- Borderless, rounded-corner, semi-transparent, always-on-top overlay at the bottom-right of the primary screen with a 50px margin, drawn entirely with GDI:
  ...
  - Before the first successful fetch, it shows the same two-row layout with 0% placeholders and `Connecting...` in the footer.
  - After a successful fetch, it keeps the last known quota visible when the network is unavailable instead of replacing it with `Connecting...`.
...
- Adaptive refresh: the first fetch starts after 100ms; failed first connections retry every 2 seconds up to 4 times, while cached data is retried every 30 seconds. Successful changes are checked every 30 seconds, and repeated unchanged results progressively slow to 2, 5, 15, then 30 minutes. A global in-flight guard skips a tick if a previous fetch is still running, so a slow or dropped network can't pile up overlapping codex subprocesses. When the machine wakes from sleep it also triggers a delayed refresh so the shown quota isn't stale.
- The last successful quota snapshot is cached at `%APPDATA%\CodexGauge\quota.json` and shown immediately on the next launch. The cache contains quota values only; it does not contain credentials or tokens.
...
- A startup refresh, adaptive refresh timers, a wake-from-sleep refresh, and manual refresh drive data updates. An in-flight guard skips a periodic tick when a previous fetch is still running, keeping at most one quota fetch in play at a time.
...
- On a failed fetch with no cached data, the widget remains in its initial `Connecting...` state and retries aggressively. If cached data exists, the last known values remain visible while the app retries in the background.
